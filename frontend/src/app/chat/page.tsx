    ...
    return (
      selectedUser && (
        <main className="flex flex-col justify-between bg-white min-h-96  h-full w-full flex-1">
          <div className="flex items-center justify-between p-4 bg-gray-300  text-black font-bold">
          ...
          {filteredMessages.map((message, index) => {
              ...
              return (
                  <div key={index} className={`flex flex-row ${message.from_user_id === user?.id ? "justify-end pl-40" : "justify-start pr-40"}`}>
                      <div className={`mt-1 rounded ${message.from_user_id === user?.id ? "bg-blue-400" : "bg-gray-400"} w-fit`}>
                         
                          <p className="px-3 mt-1 p-1 font-medium text-white rounded-md w-fit text-align-left break-all">
                              {message.message}
                          </p>
          ...
          <div className="mt-2 flex bg-white " >
            ...
            <button onClick={sendMessage} className="bg-gray-500  rounded p-2 m-2 w-40" >
              Enviar
            </button>
            </div>
        </main>
